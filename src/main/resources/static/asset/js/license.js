...
function renderActiveOrders(){
    ...
    activeOrders.forEach(order => {
        ...
        nextDate.setDate(date.getDate()+parseInt(order.pkDuration)*30);

        let today=new Date();

        html += `<tr>
              <td>${order.pkTitle}</td>
              <td>${parseInt(order.pkDuration)*30} ngày</td>
              <td>${date.getDate()}/${date.getMonth()+1}/${date.getFullYear()}</td>
...
function viewOrderDetail() {
    ...
    $btnViewOrder.each((index, btn) => {
        ...
        $(btn).on("click", function () {
            ...
            let price = parseInt(order.pkPricePerMonth) * parseInt(order.pkDuration)
            let formatprice = formatCash(price.toString());

            $("#orderDetail .user-info .name").val(order.fullName);
            $("#orderDetail .user-info .email").val(order.email);
            $("#orderDetail .user-info .phone").val(order.phone);
            $("#orderDetail .bill-package strong").text(order.pkTitle);
